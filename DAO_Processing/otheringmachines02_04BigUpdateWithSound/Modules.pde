class Module {
  ...
  float subsidingTime = 10; //time to wait after negative and positive reactions
  boolean subside = false;
  int timedStatement = int(random(60, 121));
  float respondFactor = timedStatement*0.01;
  //float stopReact = time() + 100 * respondFactor;
  float stopReact = 0;
  boolean emit=false;
  ...
  void checkIncomingData(int incomingData) {
    ...
    for (int i = 0; i < dbLength; i++) {
      if (abs(incomingData - db[i]) < freqTol) {
        ...
        println("mod "+this.modnumber+"responds in "+this.stopReact+" s");
  ...
  void timedStatement() {
    if (time() % timedStatement <= 1 && time() > stopReact) {
      ...
      println("mod "+this.modnumber+"timed statement @" + time()+ "s");
    }
  }
