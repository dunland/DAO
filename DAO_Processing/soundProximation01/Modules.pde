class Module {
  ...
  TriOsc sound = new TriOsc(soundProximation01.this); //muss sketch-Name.this sein
  ...
  void checkIncomingData(int incomingData) 
  {
    ...
    println("incomingData = "+incomingData+", ownData = "+ownData);
    
    float quotient_float = 0;
    if (incomingData > ownData)
    {
      quotient_float = float(incomingData) / float(ownData);
      println("quotient_float = "+quotient_float);
    } else if (ownData > incomingData)
    {
      quotient_float = float(ownData) / float(incomingData);
      println("quotient_float = "+quotient_float);
    }    
    int quotient_int = int(quotient_float);
    println("quotient_int = "+quotient_int);
    float decimal = quotient_float - float(quotient_int);
    println("decimal = "+decimal);

    //----------if harmonic:
    if (decimal <= 0.3 || decimal - 1 <= 0.3)
    {
      println("decimal = "+decimal);
    } 
