/*Cybernetically equalizing visuoacoustic system //<>//
 HfK Bremen
 - David Unland
 
 * gifAnimation controllable with 'g'
 * time() will be printed with 't' hit
 * always adjust TriOsc declaration in modules by referring to sketch title
 ...
 TO DO: 
 * some jitter and random noise needed
 */
...
void draw() {
  if (soundstate != STOP) {
    ...
    if (soundstate == INIT)
    {
      ...
      if (distributionMode == RANDOM)
      {
        ...
        moduleAmount++;
        println(time()+" mod "+moduleAmount+" created with number: "+modulesList.get(moduleAmount-1).ownData);
  ...
  } else if (soundstate == STOP)
  {
    ...
    textSize(40);
    textAlign(CENTER,CENTER);
    text("SIMULATION PAUSED. PRESS ENTER TO CONTINUE.", height/2,width/2);
    textSize(11);
  }
